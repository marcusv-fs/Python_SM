...
# Adicione estas classes e funções ao final ou após a função predict()
class Detector:
    """Classe para uso da rede neural sem necessidade de ROS 2"""
    _session = None
    ...
    def get_session(cls):
        if cls._session is None:
            ...
            cls._session = onnxruntime.InferenceSession(model_path, providers=providers)
        return cls._session

def detect_single_frame(frame, threshold=THRESHOLD):
    """
    Função para ser chamada por outros scripts.
    Retorna: (imagem_anotada, lista_de_deteccoes)
    """
    session = Detector.get_session()
    h, w = frame.shape[:2]
    return predict(session, frame, w, h, threshold)
